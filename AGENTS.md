...
1. **Quality Gate**: Every task must pass `bun run check` (Biome linter, TypeScript strict typecheck, Bun tests, and Rust cargo tests) with 0 errors and 0 warnings.
2. **Tri-Platform Schema Synchronization (Zero-Drift)**:
...
3. **Anti-Duplication & Single Source of Truth**:
   - Never assume or blindly create duplicate functions or constants. Always search the codebase first with `grep_search` and reuse existing helper modules (`src/lib/attendance/time-policy.ts`, `src-tauri/src/desktop/time_policy.rs`, etc.).
4. **Ironclad Backend & Sync Security**:
...
5. **Unified Stack & Android APK Readiness**:
